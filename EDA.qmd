...
A diabetes diagnosis (`Diabetes`) of "No" appears to be more tightly distributed around a lower mean. A quick t-test from a random sample of the data set shows this to a statistically significant difference. High `BMI` may help us predict a positive diagnosis.
...
The "No" class for `Diabetes` is highly peaked a 0, while the "Yes" class looks to be a bit more uniformly distributed. A quick t-test from a random sample of the data set shows a statistically significant difference. The `PhysHlth` variable seems like a good candidate to include in our prediction model.
