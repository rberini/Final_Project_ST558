...
The median values of zero, skew of \>2, and low mean values confirm the Mental Health and Physical Health variables to deviate from normality. General Health appears to be approximately normal. BMI appears to be right skewed, but the max value of 98 would be severely obese and highly unusual.

Check count of BMI values indicating extreme obesity.
...
The relative occurrence of diabetes diagnosis does not appear to differ drastically based on sex. By itself, the `Sex` variable is unlikely to be a strong predictor of `Diabetes`.
...
Similarly, the percentage "Yes" for high cholesterol is quite a bit higher than the 14% for the one-way contingency table on `Diabetes` reported earlier. This variable also may help us predict a positive diagnosis.
...
A diabetes diagnosis (`Diabetes`) of "No" appears to be more tightly distributed around a lower mean. A quick t-test below confirms this to a statistically significant difference. High `BMI` may help us predict a positive diagnosis.
...
The visual distinction between classes of `Diabetes` is less pronounced for days of poor mental health (`MentHlth`) than for physical health. That said, perhaps in conjunction with other variables, `MentHlth` can be useful is our prediction model.
...
There seems to be an association between the "Yes" class of `Diabetes` and higher age groups (`Age`). The proportion of "Yes" is greatest for "70 to 74", although generally it seems to climb with age and roughly flatten out after 65.
